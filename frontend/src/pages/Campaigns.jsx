...
function CampaignDetail({ campaign, onBack }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      // Quote requests attributed to this campaign, for when the website's
      // enquiry form is wired to record them. That table is not part of this
      // app yet, and a missing table must not blank the whole panel — leave it
      // null and the tile simply does not render.
  ...
  const cells = [
    // Only once there is one to show. An empty list here means either "nobody
    // enquired" or "that table isn't in this schema", and a permanent zero tile
    // for a loop that isn't wired up yet is worse than no tile.
    ...(quoteReqs?.length ? [['Quote requests', quoteReqs.length,
        `${quoteReqs.filter(q => q.matched_by === 'utm').length} via the link`]] : []),
  ...
  return (
    <div style={s.card}>
      ...
      <SentHistory campaignId={campaign.id} />
...
// campaign_sends is the per-recipient record: one row per person, holding the
// exact subject and body THAT person was handed plus every delivery outcome
// since. Until this existed the table was written and never read — the page
// could tell you 1,847 were sent and not who, and the one question the office
// actually gets asked ("what did you send me?") had to be answered in the SQL
// editor.
//
// Its own component rather than another select in CampaignDetail, for two
// reasons that both come from the size of a real send:
//   * body_sent is a whole email per recipient. Selecting it for 2,000 rows to
//     draw a fifty-row list would move megabytes to render names, so the list
//     query names every column EXCEPT the bodies, and one recipient's copy is
//     fetched only when their row is opened.
//   * a list you cannot filter or search is not an answer to "did Angela get
//     this one". Both run server-side, so they search the whole send rather
//     than the page currently on screen.
const SEND_STATUS_META = {
  queued:  { label: 'Queued',  bg: '#F3EFEA', fg: '#777'    },
  sending: { label: 'Sending', bg: '#FDF3E3', fg: '#7a5a12' },
  sent:    { label: 'Sent',    bg: '#E8F0E6', fg: '#3A5C2E' },
  failed:  { label: 'Failed',  bg: '#FDECEA', fg: '#C0392B' },
  skipped: { label: 'Skipped', bg: '#F3EFEA', fg: '#999'    },
}
...
// Everything except body_sent and merge_data — see the note above.
const SEND_COLS = `id, email, status, sent_at, queued_at, opened_at, open_count,
  clicked_at, click_count, error, skip_reason, subject_sent, attempts,
  marketing_contacts(first_name, last_name)`

const SENT_PAGE = 50

function applySendFilter(q, filter) {
  switch (filter) {
    case 'sent':    return q.eq('status', 'sent')
    case 'opened':  return q.not('opened_at', 'is', null)
    case 'clicked': return q.not('clicked_at', 'is', null)
    case 'failed':  return q.in('status', ['failed', 'skipped'])
    default:        return q
  }
}

function SentHistory({ campaignId }) {
  ...
  // id of the open row, and the copy fetched for it. Cached by id so reopening
  // a recipient doesn't re-fetch their email.
  const [openId, setOpenId] = useState(null)
  const [copies, setCopies] = useState({})

  // A page request that is no longer the current filter/search must not land.
  // Without this, switching filter twice quickly can leave the slower first
  // response on screen under the second filter's heading.
  ...
  async function fetchPage(from) {
    ...
    let q = supabase.from('campaign_sends')
      .select(SEND_COLS, { count: 'exact' })
      .eq('campaign_id', campaignId)
    q = applySendFilter(q, filter)
    // The address is what a person is looked up by — a support call gives you
    // an email, not a contact id. Escaped so a % or _ typed into the box is
    // matched literally instead of turning into a wildcard.
    if (search) q = q.ilike('email', `%${search.replace(/[%_\\]/g, m => '\\' + m)}%`)

    // sent_at first so the newest delivery leads, id as the tiebreak: queued and
    // skipped rows all share a NULL sent_at, and without a total order the same
    // row can come back on two pages while another never appears.
    const { data, error: err, count } = await q
      .order('sent_at', { ascending: false, nullsFirst: false })
      .order('id', { ascending: true })
      .range(from, from + SENT_PAGE - 1)

    if (req !== reqRef.current) return          // superseded
    setLoading(false)

    if (err) {
      // A campaign that has never been sent has no rows, not an error — this is
      // a real failure (RLS, a dropped column), so say so rather than showing
      // an empty list that reads as "nobody was emailed".
      setError(err.message)
      setRows(from === 0 ? [] : rows)
      return
    }
    const page = data ?? []
    setRows(from === 0 ? page : [...(rows ?? []), ...page])
    if (typeof count === 'number') setTotal(count)
    setMore(page.length === SENT_PAGE)
  }

  // Filter or search change starts the list again from the top.
  useEffect(() => { setOpenId(null); fetchPage(0) }, [campaignId, filter, search])
  ...
  async function toggleRow(row) {
    ...
    // The sender writes subject_sent and body_sent together, at the moment it
    // hands the email over, so a row with no subject never had copy rendered
    // for it (queued, or skipped) and there is nothing to go and fetch.
    if (!row.subject_sent) { setCopies(c => ({ ...c, [id]: {} })); return }

    // The list query leaves the bodies out (see the note above), so one
    // recipient's is fetched on demand. The demo tenant has no backend to fetch
    // from and seeds the copy inline, so use what the row carries when it's there.
    if (row.body_sent) {
      setCopies(c => ({ ...c, [id]: { subject_sent: row.subject_sent, body_sent: row.body_sent } }))
      return
    }

    const { data, error: err } = await supabase.from('campaign_sends')
      .select('subject_sent, body_sent').eq('id', id).maybeSingle()
    setCopies(c => ({ ...c, [id]: err ? { error: err.message } : (data ?? {}) }))
  }
  ...
  return (
    <>
      <div style={s.sentHead}>
        <div style={s.label}>Recipients</div>
        {total != null && (
          <div style={s.subtle}>
            {total === 0 ? 'None yet'
              : `${shown.toLocaleString('en-NZ')} of ${total.toLocaleString('en-NZ')}`}
          </div>
        )}
      </div>

      <div style={s.inlineRow}>
        {SEND_FILTERS.map(([id, label]) => (
          <button
            key={id}
            style={{ ...s.chip, ...(filter === id ? s.chipOn : {}) }}
            onClick={() => setFilter(id)}
          >{label}</button>
        ))}
        <form
          style={{ ...s.inlineRow, gap: 6, flex: 1, minWidth: 180 }}
          onSubmit={e => { e.preventDefault(); setSearch(searchDraft.trim()) }}
        >
          <input
            style={{ ...s.input, flex: 1, minWidth: 120 }}
            type="search" placeholder="Find an email address"
            value={searchDraft}
            onChange={e => {
              setSearchDraft(e.target.value)
              // Clearing the box (including via the × a search input draws)
              // puts the full list back without needing a second Enter.
              if (e.target.value === '') setSearch('')
            }}
          />
          <button type="submit" style={s.btnGhost}>Find</button>
        </form>
      </div>

      {error && <div style={s.errBox}>Could not read the send log: {error}</div>}

      {rows === null ? <SkeletonRows count={4} height={44} />
        : rows.length === 0 ? (
          <div style={s.empty}>
            {search || filter !== 'all'
              ? 'Nobody in this campaign matches that.'
              : 'Nothing has been sent from this campaign yet.'}
          </div>
        ) : (
          <div>
            {rows.map(r => {
              const meta = SEND_STATUS_META[r.status] ?? { label: r.status, bg: '#F3EFEA', fg: '#777' }
              const c    = r.marketing_contacts
              const who  = [c?.first_name, c?.last_name].filter(Boolean).join(' ')
              const open = openId === r.id
              const copy = copies[r.id]
              return (
                <div key={r.id} style={s.sentRow}>
                  <button style={s.sentRowBtn} onClick={() => toggleRow(r)}>
                    <span style={{ ...s.chipBadge, background: meta.bg, color: meta.fg, flexShrink: 0 }}>
                      {meta.label}
                    </span>
                    <span style={s.sentWho}>
                      {who && <span style={s.sentName}>{who}</span>}
                      <span style={s.sentEmail}>{r.email}</span>
                    </span>
                    <span style={s.sentMarks}>
                      {/* Opens and clicks per person, which is the version of
                          these numbers the stats card cannot show. */}
                      {r.opened_at  && <span title={`Opened ${fmtWhen(r.opened_at)}`}>Opened{r.open_count > 1 ? ` ×${r.open_count}` : ''}</span>}
                      {r.clicked_at && <span title={`Clicked ${fmtWhen(r.clicked_at)}`}>Clicked{r.click_count > 1 ? ` ×${r.click_count}` : ''}</span>}
                    </span>
                    <span style={s.eventWhen}>{r.sent_at ? fmtWhen(r.sent_at) : '—'}</span>
                  </button>

                  {(r.error || r.skip_reason) && (
                    <div style={s.sentProblem}>
                      {r.skip_reason ? `Skipped — ${r.skip_reason}` : r.error}
                      {r.attempts > 1 && ` · ${r.attempts} attempts`}
                    </div>
                  )}

                  {open && (
                    <div style={s.sentCopy}>
                      {copy === undefined ? <SkeletonRows count={2} height={16} />
                        : copy.error ? <div style={s.errBox}>Could not load the copy: {copy.error}</div>
                        : !copy.subject_sent && !copy.body_sent ? (
                          // Queued and skipped rows never had copy rendered for
                          // them: the sender writes subject_sent/body_sent at the
                          // moment it hands the email over.
                          <div style={s.subtle}>No copy was rendered for this recipient — nothing was sent to them.</div>
                        ) : (
                          <>
                            <div style={s.label}>Exactly what {who || r.email} was sent</div>
                            <div style={s.mail}>
                              <div style={s.mailHead}>
                                <div style={s.mailSubject}>{copy.subject_sent}</div>
                              </div>
                              <div style={s.mailBody}>{copy.body_sent}</div>
                            </div>
                          </>
                        )}
                    </div>
            ...
            {more && (
              <button style={{ ...s.btnGhost, marginTop: 8 }} disabled={loading} onClick={() => fetchPage(shown)}>
                {loading ? 'Loading…' : `Show ${SENT_PAGE} more`}
              </button>
            )}
          </div>
        )}
...
export default function Campaigns() {
  ...
  async function loadCampaigns() {
    ...
    const fresh = await Promise.all(live.map(async c => {
      ...
      // A null payload without an error is not "no activity" — it is no answer.
      // Writing it over the row would replace the snapshot the campaign already
      // carries with zeros, which reads as a campaign that reached nobody.
      return error || st == null ? null : { id: c.id, stats: st }
    }))
...
const s = {
  ...
  sentHead:   { display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' },
  sentRow:    { borderBottom: '1px solid #F5F1EB' },
  // A button, not a div, so the row is reachable by keyboard and announces
  // itself as something that opens. Reset to look like the row it replaces.
  sentRowBtn: { display: 'flex', gap: 10, alignItems: 'center', width: '100%', padding: '9px 0', border: 'none', background: 'none', textAlign: 'left', cursor: 'pointer', fontFamily: 'var(--font)', fontSize: 12.5 },
  sentWho:    { display: 'flex', flexDirection: 'column', gap: 1, flex: 1, minWidth: 0 },
  sentName:   { fontWeight: 700, color: 'var(--ink)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  sentEmail:  { color: '#8a8a8a', fontSize: 11.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  sentMarks:  { display: 'flex', gap: 8, flexShrink: 0, fontSize: 11, color: 'var(--terra)', fontWeight: 700 },
  sentProblem:{ fontSize: 11.5, color: '#C0392B', lineHeight: 1.5, padding: '0 0 9px' },
  sentCopy:   { display: 'flex', flexDirection: 'column', gap: 6, padding: '4px 0 14px' },
